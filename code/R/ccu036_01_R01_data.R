...
# input data --------------------------------------------------------------
## .ccu036_01_out_cohort
## .ccu036_01_out_covariates
## .ccu036_01_out_exposure
## .ccu036_01_out_outcomes_dur_preg
## .ccu036_01_out_outcomes_post_preg
## .ccu036_01_out_outcomes_at_birth
## .ccu036_01_out_skinny
## .ccu036_01_out_covid
## .ccu036_01_cohort_jcvi
...
# cohort
ccu036_cohort_tbl <- tbl(con, in_schema("", "ccu036_01_out_cohort"))
cohort <- ccu036_cohort_tbl %>%
  collect()

# covariates
ccu036_covariates_tbl <- tbl(con, in_schema("", "ccu036_01_out_covariates"))
covariates <- ccu036_covariates_tbl %>%
  collect()

# covid exposure
ccu036_exposure_tbl <- tbl(con, in_schema("", "ccu036_01_out_exposure"))
exposure <- ccu036_exposure_tbl %>%
 collect()

# covid vaccination (NOT NEEDED AS WE HAVE EXPOSURE TABLE)
# ccu036_vaccination_tbl <- tbl(con, in_schema("", "ccu036_01_vaccination"))
# vaccination <- ccu036_vaccination_tbl %>%
#   collect()

# outcomes during pregnancy
ccu036_outcomes_dur_preg_tbl <- tbl(con, in_schema("", "ccu036_01_out_outcomes_dur_preg"))
outcomes_during_preg <- ccu036_outcomes_dur_preg_tbl %>%
  collect()

# outcomes post pregnancy
ccu036_outcomes_post_preg_tbl <- tbl(con, in_schema("", "ccu036_01_out_outcomes_post_preg"))
outcomes_post_preg <- ccu036_outcomes_post_preg_tbl %>%
  collect()

# outcomes at birth
ccu036_outcomes_at_birth_tbl <- tbl(con, in_schema("", "ccu036_01_out_outcomes_at_birth"))
outcomes_birth <- ccu036_outcomes_at_birth_tbl %>%
  collect()

# skinny table
ccu036_skinny_tbl <- tbl(con, in_schema("", "ccu036_01_out_skinny"))
skinny <- ccu036_skinny_tbl %>%
  collect()

#during pregnancy covid infection
ccu036_covid<- tbl(con, in_schema("", "ccu036_01_out_covid"))
skinny <- ccu036_covid %>%
  collect()
covid <- skinny
...
data <- dbGetQuery(con,'SELECT * FROM .ccu036_01_cohort_jcvi')
