...
### "ccu036_01_analysis_data_04122022.csv"  # limited pregnancies with start date between 08 Dec 2020 and 31 Aug 2021
...
data <- read.csv("D:/PhotonUser/My Files/Home Folder/collab/CCU036_01/data/ccu036_01_analysis_data_04122022.csv") 
...
date <- '07_09_2023' #(vacc_covid_r) #"07_09_2023":(vacc_covid_r), '08_09_2023' :(vacc_az_r), '09_09_2023': (vacc_pf_r) ,  '10_09_2023' :(vacc2d_24wk), '11_09_2023': (vacc3d_24wk) 
...
cox_model_full <- function(k, y, z) {
  ...
  data <- data %>%
    dplyr::mutate(overall_event1 = case_when(data$overall_event == 1 & data$exp_dur_covid_1st_date< data$overall_pregn_event_date  & data$exp_dur_covid_1st_date>fup_start   & !is.na(data$exp_dur_covid_1st_date)~  0,
  ...
  data <- data %>% 
    dplyr::mutate(fup_end=case_when(data$overall_event1 == 1 ~  data$overall_pregn_event_date1,
                                    ...
                                    data$overall_event1 == 0  & overall_event==0 ~ data$DELIVERY_DATE
    )
    ) 
  ...
  data$overall_event_time<-as.numeric(difftime(data$overall_pregn_event_date1, data$fup_start, unit="days"))
  ...
  data$vacc_covid <- data$vacc_covid_r  #"07_09_2023":(vacc_covid_r), '08_09_2023' :(vacc_az_r), '09_09_2023': (vacc_pf_r) ,  '10_09_2023' :(vacc2d_24wk), '11_09_2023': (vacc3d_24wk) 
...
################  remove the rows with fup days less than 0#####################
  ...
  td_data <-
    tmerge(
      ...
      data2 = vacc_cases %>% select(all_of(vars), overall_event1, fup,delta_vacc_covid),
      id = id,
      overall_event = event(fup,  overall_event1)#,
